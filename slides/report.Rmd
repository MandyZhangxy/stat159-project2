...
## 1. Ordinary Least Square Method (cont.)
...
where $\lambda \geq 0$ is a *tunig parameter*, to be determined separately. Equation trades off two different criteria. 

## 2.1 Ridge Regression (cont.)
...
## 3. Dimension Reduction Method (cont.)
...
In general, one can construct up to p distinct principal components. The second principal component $Z_2$ is linear combination of the variables that is uncorrelated with $Z_1$, and has largest variance subject to this constraint. 

## 3.1 Principal Components Regression (cont.)
With more predictors, then more additional componetns could be constructed. They would successively maximize variance, subject to the constraint of being uncorrelated with the preceding components. 
...
## 3.2 Partial Least Square Regression (cont.)
...
## Pre-modeling Data Processing (cont.)
...
print(pcrcoef,comment = FALSE, type = "html")
```
...
print(plsrcoef,comment = FALSE, type = "html")
```
...
print(aa, comment = FALSE, type = "html")
```

## Comparing the Coefficient Estimates for 5 regression models (cont.)
...
print(bb, comment = FALSE, type = "html")
```

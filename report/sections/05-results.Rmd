...
lassocoef <- xtable(lasso_coefficients, caption = "Lasso Coefficients", digits = c(0,5))
...
##PCR
Now we use a different method which focus on dimension reduciton by unsupervised learning -- the principle component methods to fit on the training data. In this case, we think the optimal number of principle components used is `r pcr_best`, and the resulting test MSE is `r pcr_mse`.
...
pcrcoef <- xtable(pcr_coef, caption = "PCR Coefficients",digits = c(0,5))
...
plsrcoef <- xtable(plsr_coef, caption = "PLSR Coefficients", digits = c(0,5))
